...
val finalDF = total_mentions_events.groupBy("Total_mentions.EventId","Actor1Name","Actor2Name","EventCode","QuadClass","daymonthevent","SOURCEURL").select(col("EventId"), col("Actor1Name"),col("Actor2Name"),col("EventCode"),col("QuadClass"),col("daymonthevent"),col("SOURCEURL"))

MongoSpark.save(finalDF.write.option("spark.mongodb.output.uri", "mongodb://master:27020/gdelt.query2").mode("overwrite"))
