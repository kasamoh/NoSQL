...
val finalDF = total_mentions_events.groupBy("yearmention","monthmention","daymonthevent","ActionGeo_CountryCode","MentionDocTranslationInfo").agg(sum("NumMentions").alias("nb_articles"))


MongoSpark.save(finalDF.write.option("spark.mongodb.output.uri", "mongodb://master:27020/gdelt.query1").mode("overwrite"))
