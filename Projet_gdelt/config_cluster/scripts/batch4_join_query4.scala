...
val finalDF = total_mentions_events.groupBy("total_mentions.EventId","ActionGeo_CountryCode").agg(avg("GoldsteinScale").alias("avgGoldsteinScale")).groupBy("ActionGeo_CountryCode").agg(sum("avgGoldsteinScale").alias("GoldsteinScale")).select(col("ActionGeo_CountryCode"), col("GoldsteinScale"))


MongoSpark.save(finalDF.write.option("spark.mongodb.output.uri", "mongodb://master:27020/gdelt.total_goldstein").mode("overwrite"))
